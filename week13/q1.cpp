...
int main(){
    ...
    MaxHeap<Pair<int,int> > pq(2*n, comp);
    int st;cin >> st;
    pq.add(Pair<int,int>({0, st}));
    Vector<int> vis(n, 0);
    lev[st] = 0;
    ...
    while (pq.size() > 0){
        ...
        for (int i=0;i<graph[curr.second].size();i++){
            auto ele = graph[curr.second][i];
            if (vis[ele.first]) continue;
            if (lev[ele.first] > ele.second + curr.first){
                lev[ele.first] = ele.second + curr.first;
                pq.add(Pair<int,int>({lev[ele.first], ele.first}));
            }
        }
    }
    for (int i=0;i<n;i++){
